...
bool PipeWireSourceStream::createStream(uint nodeid)
{
    ...
    const spa_pod *param = (spa_pod *)spa_pod_builder_add_object(&podBuilder,
                                                                 ...
                                                                 SPA_POD_CHOICE_ENUM_Id(6, SPA_VIDEO_FORMAT_RGBx, SPA_VIDEO_FORMAT_RGBA, SPA_VIDEO_FORMAT_BGRx, SPA_VIDEO_FORMAT_BGRA, SPA_VIDEO_FORMAT_RGB, SPA_VIDEO_FORMAT_BGR),
...
void PipeWireSourceStream::handleFrame(struct pw_buffer *buffer)
{
    ...
    if (spaBuffer->datas->type == SPA_DATA_MemFd) {
        uint8_t *map = static_cast<uint8_t *>(mmap(nullptr, spaBuffer->datas->maxsize + spaBuffer->datas->mapoffset, PROT_READ, MAP_PRIVATE, spaBuffer->datas->fd, 0));
