...
void TaskGroupingProxyModel::Private::sourceDataChanged(QModelIndex topLeft, QModelIndex bottomRight, const QVector<int> &roles)
{
    for (int i = topLeft.row(); i <= bottomRight.row(); ++i) {
        ...
        if (!parent.isValid() && !groupDemandingAttention && roles.contains(AbstractTasksModel::IsDemandingAttention) && !sourceIndex.data(AbstractTasksModel::IsDemandingAttention).toBool()) {
...
void TaskGroupingProxyModel::setSourceModel(QAbstractItemModel *sourceModel)
{
    ...
    if (sourceModel) {
        ...
        auto dd = d.data();
        connect(sourceModel, &QSortFilterProxyModel::rowsAboutToBeInserted, this, std::bind(&TaskGroupingProxyModel::Private::sourceRowsAboutToBeInserted, dd, _1, _2, _3));
        ...
        connect(sourceModel, &QSortFilterProxyModel::rowsAboutToBeRemoved, this, std::bind(&TaskGroupingProxyModel::Private::sourceRowsAboutToBeRemoved, dd, _1, _2, _3));
...
void TaskGroupingProxyModel::requestToggleGrouping(const QModelIndex &index)
{
    ...
    for (int i = 0; i < d->rowMap.count(); ++i) {
        if (!d->isGroup(i)) {
            ...
            if (idx.data(AbstractTasksModel::AppId).toString() == appId || launcherUrlsMatch(idx.data(AbstractTasksModel::LauncherUrlWithoutIcon).toUrl(), launcherUrl, IgnoreQueryItems)) {
