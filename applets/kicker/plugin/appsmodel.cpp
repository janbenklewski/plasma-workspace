    ...
    , m_appNameFormat(AppEntry::NameOnly)
{
    foreach (AbstractEntry *suggestedEntry, entryList) {
        ...
        foreach (const AbstractEntry *entry, m_entryList) {
            if (entry->type() == AbstractEntry::RunnableType && static_cast<const AppEntry *>(entry)->service()->storageId() == static_cast<const AppEntry *>(suggestedEntry)->service()->storageId()) {
                found = true;
                break;
            }
        }
    ...
    if (actionId == QLatin1String("hideApplication") && entry->type() == AbstractEntry::RunnableType) {
        ...
        if (appletConfig && appletConfig->contains(QLatin1String("hiddenApplications"))) {
            ...
            if (!hiddenApps.contains(service->menuId())) {
                ...
                QMetaObject::invokeMethod(appletConfig, "valueChanged", Qt::DirectConnection, Q_ARG(QString, QStringLiteral("hiddenApplications")), Q_ARG(QVariant, hiddenApps));
...
void AppsModel::processServiceGroup(KServiceGroup::Ptr group)
{
    ...
    KServiceGroup::List list = group->entries(true /* sorted */, true /* excludeNoDisplay */, (!m_flat || (m_flat && !hasSubGroups)) /* allowSeparators */, sortByGenericName /* sortByGenericName */);
