    ...
    , m_objectPath(objectPath)
{
    ...
    if (!QDBusConnection::sessionBus().connect(m_serviceName, m_objectPath, s_orgGtkMenus, QStringLiteral("Changed"), this, SLOT(onMenuChanged(GMenuChangeList)))) {
...
void Menu::stop(const QList<uint> &ids)
{
    ...
    connect(watcher, &QDBusPendingCallWatcher::finished, this, [this, ids](QDBusPendingCallWatcher *watcher) {
        ...
        if (reply.isError()) {
            ...
            m_subscriptions.erase(std::remove_if(m_subscriptions.begin(), m_subscriptions.end(), std::bind(&QList<uint>::contains, m_subscriptions, std::placeholders::_1)), m_subscriptions.end());
