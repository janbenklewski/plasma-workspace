...
QString GridLayoutManager::serializeLayout() const
{
    ...
    for (auto *item : layout()->childItems()) {
        ...
        if (itemCont && itemCont != layout()->placeHolder()) {
            result += itemCont->key() + QLatin1Char(':') + QString::number(itemCont->x()) + QLatin1Char(',') + QString::number(itemCont->y()) + QLatin1Char(',') + QString::number(itemCont->width()) + QLatin1Char(',') +
                QString::number(itemCont->height()) + QLatin1Char(',') + QString::number(itemCont->rotation()) + QLatin1Char(';');
        }
    }
...
bool GridLayoutManager::assignSpaceImpl(ItemContainer *item)
{
    ...
    for (auto *i2 : layout()->childItems()) {
        ...
        if (item2 && item2->parentItem() == item->parentItem() && item != item2 && item2 != layout()->placeHolder() && item->y() < item2->y() + item2->height() && item->x() <= item2->x()) {
