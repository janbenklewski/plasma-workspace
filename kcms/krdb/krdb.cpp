...
void runRdb(uint flags)
{
    ...
    if (exportColors) {
        ...
        const QStringList adPaths = QStandardPaths::locateAll(QStandardPaths::GenericDataLocation, QStringLiteral("kdisplay/app-defaults/"), QStandardPaths::LocateDirectory);
    ...
    if (exportQtColors || exportQtSettings) {
        ...
        if (qApp->platformName() == QLatin1String("xcb")) {
            ...
            s << settingsstamp;
            XChangeProperty(QX11Info::display(), QX11Info::appRootWindow(), qt_settings_timestamp, qt_settings_timestamp, 8, PropModeReplace, (unsigned char *)stamp.buffer().data(), stamp.buffer().size());
            QApplication::flush();
        }
#endif
    }
