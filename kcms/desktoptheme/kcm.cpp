...
void KCMDesktopTheme::installTheme(const QString &path)
{
    ...
    connect(myProcess, static_cast<void (QProcess::*)(int, QProcess::ExitStatus)>(&QProcess::finished), this, [this](int exitCode, QProcess::ExitStatus exitStatus) {
        Q_UNUSED(exitStatus)
        if (exitCode == 0) {
...
void KCMDesktopTheme::processPendingDeletions()
{
    ...
    for (const QPersistentModelIndex &idx : persistentPendingDeletions) {
        ...
        connect(process, static_cast<void (QProcess::*)(int, QProcess::ExitStatus)>(&QProcess::finished), this, [this, process, idx, pluginName, displayName](int exitCode, QProcess::ExitStatus exitStatus) {
            Q_UNUSED(exitStatus)
            if (exitCode == 0) {
