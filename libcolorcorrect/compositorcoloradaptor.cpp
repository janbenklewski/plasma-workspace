...
CompositorAdaptor::CompositorAdaptor(QObject *parent)
    : QObject(parent)
{
    m_iface = new QDBusInterface(QStringLiteral("org.kde.KWin"), QStringLiteral("/ColorCorrect"), QStringLiteral("org.kde.kwin.ColorCorrect"), QDBusConnection::sessionBus(), this);

    if (!m_iface->connection().connect(QString(), QStringLiteral("/ColorCorrect"), QStringLiteral("org.kde.kwin.ColorCorrect"), QStringLiteral("nightColorConfigChanged"), this, SLOT(compDataUpdated(QHash<QString, QVariant>)))) {
...
bool CompositorAdaptor::checkStaged()
{
    ...
    case Mode::ModeTimings:
        return baseDataChange || m_morningBeginFixed != m_morningBeginFixedStaged || m_eveningBeginFixed != m_eveningBeginFixedStaged || m_transitionTime != m_transitionTimeStaged;
...
{
    return m_active != m_activeStaged || m_mode != m_modeStaged || m_nightTemperature != m_nightTemperatureStaged || m_latitudeFixed != m_latitudeFixedStaged || m_longitudeFixed != m_longitudeFixedStaged ||
        m_morningBeginFixed != m_morningBeginFixedStaged || m_eveningBeginFixed != m_eveningBeginFixedStaged || m_transitionTime != m_transitionTimeStaged;
}
