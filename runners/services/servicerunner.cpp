...
private:
    ...
    QString generateQuery(const QVector<QStringRef> &strList)
    {
        ...
        QString finalQuery = QStringLiteral("exist Exec and ( (%1) or (%2) or (%3) or ('%4' ~~ Exec) or (%5) )").arg(keywordTemplate, genericNameTemplate, nameTemplate, strList[0].toString(), commentTemplate);
    ...
    void matchJumpListActions()
    {
        ...
        for (const KService::Ptr &service : services) {
            ...
            for (const KServiceAction &action : actions) {
                ...
                match.setText(i18nc("Jump list search result, %1 is action (eg. open new tab), %2 is application (eg. browser)", "%1 - %2", action.text(), service->name()));
