    ...
    , m_withStatusNotifier(false)
{
    ...
    QQmlExpression *expr = new QQmlExpression(engine()->rootContext(),
                                              contentItem(),
                                              QStringLiteral("Qt.createQmlObject('import QtQuick 2.0; import QtQuick.Layouts 1.1; import org.kde.plasma.core 2.0; Rectangle {color: theme.backgroundColor; anchors.fill:parent; property Item "
                                                             "appletInterface; onAppletInterfaceChanged: print(appletInterface.Layout.minimumWidth)}', root, \"\");"));
...
void PlasmaWindowedView::setApplet(Plasma::Applet *applet)
{
    ...
    foreach (QObject *child, m_appletInterface->children()) {
        ...
        if (child->property("minimumWidth").isValid() && child->property("minimumHeight").isValid() && child->property("preferredWidth").isValid() && child->property("preferredHeight").isValid() &&
            child->property("maximumWidth").isValid() && child->property("maximumHeight").isValid() && child->property("fillWidth").isValid() && child->property("fillHeight").isValid()) {
            m_layout = child;
        }
    }
