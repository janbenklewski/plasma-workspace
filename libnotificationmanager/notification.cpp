...
QString Notification::Private::sanitize(const QString &text)
{
    ...
    if (r.hasError()) {
        qCWarning(NOTIFICATIONMANAGER) << "Notification to send to backend contains invalid XML: " << r.errorString() << "line" << r.lineNumber() << "col" << r.columnNumber();
    }
...
void Notification::Private::setDesktopEntry(const QString &desktopEntry)
{
    ...
    if (!notifyRcName.isEmpty()) {
        ...
        config.addConfigSources(QStandardPaths::locateAll(QStandardPaths::GenericDataLocation, QStringLiteral("knotifications5/") + notifyRcName + QStringLiteral(".notifyrc")));
