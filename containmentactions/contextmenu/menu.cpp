...
void ContextMenu::restore(const KConfigGroup &config)
{
    ...
    if (c->containmentType() == Plasma::Types::PanelContainment || c->containmentType() == Plasma::Types::CustomPanelContainment) {
        ...
        m_actionOrder << QStringLiteral("configure") << QStringLiteral("run associated application") << QStringLiteral("configure shortcuts") << QStringLiteral("_sep1") << QStringLiteral("_context") << QStringLiteral("_run_command")
                      << QStringLiteral("add widgets") << QStringLiteral("_add panel") << QStringLiteral("manage activities") << QStringLiteral("remove") << QStringLiteral("edit mode") << QStringLiteral("_sep2")
                      << QStringLiteral("_lock_screen") << QStringLiteral("_logout") << QStringLiteral("_sep3") << QStringLiteral("_wallpaper");
    ...
    foreach (const QString &name, m_actionOrder) {
        ...
        if (name == QLatin1String("_wallpaper")) {
            ...
            if (name != QLatin1String("remove") || c->isUserConfiguring() ||
                (c->containmentType() != Plasma::Types::PanelContainment && c->containmentType() != Plasma::Types::CustomPanelContainment && c->corona()->immutability() != Plasma::Types::Mutable)) {
                actions << a;
            }
        }
    }
