...
void UKMETIon::findPlace(const QString &place, const QString &source)
{
    ...
    const QUrl url(QLatin1String("https://www.bbc.com/locator/default/en-GB/search.json?search=") + place + QLatin1String("&filter=international&postcode_unit=false&postcode_district=true"));
...
void UKMETIon::updateWeather(const QString &source)
{
    ...
    for (const WeatherData::ForecastInfo *forecastInfo : forecasts) {
        ...
        data.insert(QStringLiteral("Short Forecast Day %1").arg(i), QStringLiteral("%1|%2|%3|%4|%5|%6").arg(period, forecastInfo->iconName, forecastInfo->summary, tempHigh, tempLow, QString()));
