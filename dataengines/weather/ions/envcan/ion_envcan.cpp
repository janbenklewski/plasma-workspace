...
void EnvCanadaIon::updateWeather(const QString &source)
{
    ...
    for (const WeatherData::ForecastInfo *forecastInfo : forecasts) {
        ...
        const QString shortForecast = forecastInfo->shortForecast.isEmpty() ? i18n("N/A") : i18nc("weather forecast", forecastInfo->shortForecast.toUtf8().data());
        ...
        data.insert(QStringLiteral("Short Forecast Day %1").arg(i), QStringLiteral("%1|%2|%3|%4|%5|%6").arg(forecastPeriod, forecastInfo->iconName, shortForecast, tempHigh, tempLow, popPrecent));
        ++i;
    }
